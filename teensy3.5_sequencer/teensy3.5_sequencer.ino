...
void setup() {
  ...
  analogWriteFrequency(5, frequencies[0]);
  analogWriteFrequency(3, frequencies[1]);
  analogWriteFrequency(29,frequencies[2]);
  analogWriteFrequency(2, frequencies[3]);
...

{
  ...
  uint8_t otpt;
  for(uint8_t i;i<8;i++) { 
    if(sequence_data[Step][i] == 9) { digitalWrite(pin[i],HIGH);}
    else { otpt = map(sequence_data[Step][i],0,9,0,255);analogWrite(pin[i],otpt);}   
   } 
}
